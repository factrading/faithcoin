...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::FAC);
    ...
    if (!settings.contains("nAnonymizeFacAmount"))
        settings.setValue("nAnonymizeFacAmount", 1000);

    nObfuscationRounds = settings.value("nObfuscationRounds").toLongLong();
    nAnonymizeFacAmount = settings.value("nAnonymizeFacAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeFacAmount"))
        SoftSetArg("-anonymizefacamount", settings.value("nAnonymizeFacAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in fac.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeFacAmount:
            return QVariant(nAnonymizeFacAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case ObfuscationRounds:
            ...
            break;
        case AnonymizeFacAmount:
            nAnonymizeFacAmount = value.toInt();
            settings.setValue("nAnonymizeFacAmount", nAnonymizeFacAmount);
            emit anonymizeFacAmountChanged(nAnonymizeFacAmount);
            break;
        case CoinControlFeatures:
